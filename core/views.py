...
def profile_edit(request):
	...
	if request.method == 'POST':
		...
		form_verify  = VerificationUploadForm(request.POST, request.FILES)

		if all([form_fixed.is_valid(), form_dynamic.is_valid(), form_verify.is_valid()]):
			with transaction.atomic():
				...
				for field, doc_type in (
					('doc_identify',	VerificationSubmission.DocType.IDENTIFY),
					('doc_single',		VerificationSubmission.DocType.SINGLE),
					('doc_income',		VerificationSubmission.DocType.INCOME),
					('doc_graduate',	VerificationSubmission.DocType.GRADUATE),
				):
					file = form_verify.cleaned_data.get(field)
					if file:
						VerificationSubmission.objects.update_or_create(
							user=request.user,
							doc_type=doc_type,
							defaults={
								'image'		: file,
		...
		form_verify  = VerificationUploadForm()
